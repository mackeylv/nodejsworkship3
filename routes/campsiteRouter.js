...
//since we explicitly wrote the path in server.js, we didn't have to write it out completely in this .js file. We only needed '/'.
...
//Workshop 1-Task 1: Complete the updates to campsiteRouter 
...
.delete(authenticate.verifyUser, authenticate.verifyAdmin, (req, res, next) => {    /*Workshop 3-Task 2: Set up admin-only access points*/
    ...
    .then(campsite => {
        if (campsite) {
            //to iterate through each comment for the campsite is why we use a 'For' loop
...
//for the specific comment based on a specific campsite
...
.put(authenticate.verifyUser, (req, res, next) => {
    ...
    .then(campsite => {
        if (campsite && campsite.comments.id(req.params.commentId)) { //Only allow updates of specific fields--rating, text 
            //Allows only certain authors that Post, to edit their Comment 
